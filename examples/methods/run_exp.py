...
import argparse
from selfrag_utils import load_jsonlines, preprocess_input_data
...
def LlamaIndexIter(args):
    ...
    # save_note = f'{pipe_version}_topk_{args.retrieval_topk}_nprobe_{args.retrieval_nprobe}'
    save_note = f'topk_{args.retrieval_topk}_nprobe_{args.retrieval_nprobe}'
    config_dict = {
        'method_name': "llamaindex_iter",
        'save_note': save_note,
        'gpu_id':args.gpu_id,
        'dataset_name':args.dataset_name,
        'retrieval_batch_size': args.batch_size,
...
def LlamaIndexSubQuestion(args):
    save_note = f'topk_{args.retrieval_topk}_nprobe_{args.retrieval_nprobe}'
    config_dict = {
        'method_name': "llamaindex_subquestion",
        'save_note': save_note,
        'gpu_id':args.gpu_id,
        'dataset_name':args.dataset_name,
        'retrieval_batch_size': args.batch_size,
    ...
    from flashrag.pipeline import LlamaIndexSubquestionPipeline
    ...
    pipeline = LlamaIndexSubquestionPipeline(config, num_subquestion=3)
...
def selfrag(args):
    ...
    config_dict = {
        ...
        'generator_model_path': '/data/llama2/selfrag_llama2_7b',
        'framework': 'vllm',
        'save_note': save_note,
        'gpu_id': args.gpu_id,
...
def selfrag_align(args):
    """
    Utilize Self-RAG preproceeded data to align the accuracy
    """
    save_note = f'topk_{args.retrieval_topk}_nprobe_{args.retrieval_nprobe}_align'
    config_dict = {
        'method_name': args.method_name,
        'generator_model':'selfrag-llama2-7B',
        'generator_model_path': '/data/llama2/selfrag_llama2_7b',
    ...
    input_path = "/home/cyulin/rag/self-rag/retrieval_lm/eval_data/triviaqa_test.jsonl"
    align_data = load_jsonlines(input_path)
    align_data = preprocess_input_data(align_data)

    from flashrag.pipeline import SelfRAGPipeline2
    pipeline = SelfRAGPipeline2(config, threhsold=0.2, max_depth=2, beam_width=2,
                                w_rel=1.0, w_sup=1.0, w_use=1.0,
                                use_grounding=True, use_utility=True, use_seqscore=True, ignore_cont=True,
                                mode='adaptive_retrieval')
    result = pipeline.run(test_data, align_data)
...
def flare(args):
    ...
    from flashrag.pipeline import FLAREPipeline2
    pipeline = FLAREPipeline2(config, min_prob=args.min_prob, max_iter_num=4)
    result = pipeline.run(test_data)
...
if __name__ == '__main__':
    ...
    func_dict = {
        ...
        'skr': skr,
        'selfrag': selfrag,
        'selfrag_align': selfrag_align,
        'flare': flare,
        'iterretgen': iterretgen,
        'hyde': hyde,
        'llamaIdx-iter': LlamaIndexIter,
        'llamaIdx-sub': LlamaIndexSubQuestion
    }
