...
class OpenaiGenerator:
    ...
    def generate(self, input_list: List[List], batch_size=None, return_scores=False, return_tokens=False, **params) -> List[str]:
        ...
        if "max_new_tokens" in generation_params.keys():
        ...
        for res in result:
            ...
            if return_tokens:
                token = list(map(lambda x: x.token, res.logprobs.content))
                tokens.append(token)
        if return_scores:
            if return_tokens:
                return response_text, scores, tokens
            else:
