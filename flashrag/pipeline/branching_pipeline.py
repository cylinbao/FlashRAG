...
class LlamaIndexSubquestionPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, num_subquestion=3):
        from flashrag.pipeline.replug_utils import load_replug_model
        super().__init__(config, prompt_template)

        self.num_subquestion = num_subquestion
        self.retriever = get_retriever(config)
        self.generator = get_generator(config)

        from flashrag.prompt import PromptTemplate
        from flashrag.prompt import DEFAULT_TREE_SUMMARIZE_TMPL
        from flashrag.prompt import DEFAULT_SUBQUESTION_SYSTEM_TMPL

        self.subquestion_template = PromptTemplate(
            config = config,
            system_prompt = DEFAULT_SUBQUESTION_SYSTEM_TMPL,
        )
    ...
    def extract_questions(self, query_list):
    ...
    def run_item(self, item):
        ...
        subquestion_prompts = [
            self.subquestion_template.get_string(
            question=question,
        )]
        item.update_output('subquestion_prompt', subquestion_prompts[0][0])

        subquestion_query = self.generator.generate(
            subquestion_prompts,
        )
        item.update_output('raw_subquestions_string', subquestion_query[0])
        generated_questions = self.extract_questions(subquestion_query[0])

        # restrict max generated questions to 5
        if len(generated_questions) > 5:
            generated_questions = generated_questions[:5]
        # item.update_output('extract_subquestions', generated_questions)
        ...
        item.update_output('retrieval_queries', generated_questions)
        item.update_output('combined_retrieval_results', retrieval_results)

        input_prompts = [self.prompt_template.get_string(
            question=question, retrieval_result=retrieval_results
        )]
        item.update_output('llm_prompt', input_prompts[0][0])
        generation_result = self.generator.generate(input_prompts)
        # item.update_output('llm_response', generation_result)
